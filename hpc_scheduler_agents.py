...
==================================
Two-agent system for VM/container placement:
  - Agent PLACER : Power-first placement with affinity slack (Rule T)
  - Agent RA     : Post-placement Pareto-improving rearrangement (Move / Swap)
...
class HPCSchedulerOrchestrator:
    ...
    def run_iteration(self, dec_w, step_w) -> bool:
        ...
        if ops.startswith("(No"):
            ...
            rres = self.agent_ra.call(self._ra_prompt())
            ...
            if rd and rd.get("action") == "Move":
                ...
                swap_required = ["instance_id1","app_id1","machine1","instance_id2","app_id2","machine2"]
    ...
    def run(self) -> Dict[str, Any]:
        self.log("="*70)
        self.log("HPC Scheduler — PLACER + RA (Power primary, Affinity secondary)")
        self.log("="*70)
