...
==================================
Affinity-Only scheduler: always picks the highest-affinity feasible placement.
No power consideration. No RA agent.
...
PLACER_PROMPT = r"""You are an affinity-maximizing cluster scheduler. Your ONLY goal is maximum affinity.
...
GOAL: Maximize affinity (prefer affinity=1 placements). Power is IGNORED.

DECISION POLICY — Affinity-only (strict):
- All rows in the candidate table are already feasible.
- First choose candidates with affinity=1. Among those, pick the one with smallest delta_total as tiebreaker.
- If no affinity=1 candidate exists, pick any candidate (smallest delta_total).
- Ignore power minimization completely.

IMPORTANT: Output ONLY a JSON object — no markdown, no explanation, no code fences.

Output schemas:
{{"action":"Place","app_id":int,"machine_id":int,"dp_min":float,"epsilon":0,"chosen_dp":float,"chosen_affinity":int,"rationale":"..."}}
...
class HPCSchedulerOrchestrator:
    ...
    def run(self) -> Dict[str, Any]:
        self.log("="*70)
        self.log("HPC Scheduler — AFFINITY ONLY (no power opt, no RA)")
        self.log("="*70)
